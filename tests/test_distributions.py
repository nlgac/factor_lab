...
the raw form on opt-out, loc/df preserved, and df<=2 rejected when standardizing.
"""
...
SEED = 20240621
N = 4_000_000   # large enough for a tight variance estimate
...
class TestStudentTStandardization:
    ...
    @pytest.mark.parametrize("df", [3, 5, 6, 30])
    def test_default_is_unit_variance(self, df):
        """standardize defaults to True: realized variance ~= 1 for any df>2."""
