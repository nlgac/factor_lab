...
\title{Multifactor Dispersion Bias with Per-Column Prevalence: A Unified Treatment}
\author{}
\date{}
...
\textit{This document consolidates two complementary lines of work: the k-factor dispersion bias result (Theorem 3.1+, referred to as \textbf{NG} below, where NG denotes the author of that result) and the multifactor prevalence extension (Theorems 1 and 3 of the companion paper, abbreviated \textbf{MPE} and referred to below by its author as \textbf{AK}). The result is a single, fully proved theorem that subsumes both, with Part (i) following the NG projection argument and Part (ii) following the AK proof architecture, translated into unified notation. Term definitions appear in §2; the introduction uses terms informally.}

\textit{Author abbreviations: \textbf{NG} = [Nick Gunther]; \textbf{AK} = [Alec Kircheval]. Full citations appear in the bibliography.}
...
\textbf{Data model.} The $p\times n$ data matrix is
...
\textbf{Signal subspace.} Write $\mathcal{B} = \mathrm{col}(B) \subset \mathbb{R}^p$ for the $k$-dimensional population factor subspace, $\Pi_B = B(B^\top B)^{-1}B^\top$ for the orthogonal projection onto $\mathcal{B}$, and $\Pi_B^\perp = I_p - \Pi_B$ for its complement.

\textbf{SVD of normalized loading matrix.} Fix the thin SVD
...
\textbf{Prevalence-rescaled factor returns.} Define the \textit{prevalence-rescaled factor return matrix}

\[
F^\# = C^{1/2}F^\top \in \mathbb{R}^{k\times n}.
\]

\textbf{Prevalence-weighted factor covariance matrices.} Define the $k\times k$ matrices
...
\textit{Intuition.} $\hat{D}$ blends two sources of information: how strongly each factor is spread cross-sectionally (captured by $C^{1/2}$) and how much it varies over time (captured by $F^\top F/n$). The diagonal entry $\hat{D}_{jj} = c_j\cdot (F_j^\top F_j/n)$ is the $j$-th factor's per-period contribution to cross-sectional variance.

\textit{Remark on loading norms.} The diagonal matrix $A(p) = \mathrm{diag}(\Vert \beta_j(p)\Vert )$ satisfies $A(p)/\sqrt{p} \to C^{1/2}$ by Assumption 1. Equivalently, $B^\top B/p = A(p)G(p)A(p)/p \to C^{1/2}G_\infty C^{1/2} = \Gamma_B$. This convergence $\Gamma_p \to \Gamma_B$ is all that the proof of §7 requires; the matrices $A(p)$ and $G(p)$ enter only through their product.

\textbf{Rotated-and-reweighted matrices for general $G_\infty$.} Fix a spectral decomposition
...
\textbf{Population loading direction.} Let $\bar{b}_j$ denote the unit eigenvector of the $p\times p$ population signal covariance
...
\textbf{Symbol summary.}

\textit{Grouping follows the order of introduction in §2. Items first defined outside §2 note their location.}
...
Symbol & Meaning \\
\hline
$A(p)$ & Diagonal matrix of loading-column norms: $A(p) = \mathrm{diag}(\Vert \beta_j(p)\Vert )_{j=1}^k$ \\
\hline
$b(p) = BA(p)^{-1}$ & Unit-normalized loading matrix; columns are $\beta_j/\Vert \beta_j\Vert $ \\
\hline
...
\hline
$c_j$, $C$ & Prevalence of factor $j$: $c_j = \lim\Vert \beta_j\Vert^2/p \in (0,\infty)$; $C = \mathrm{diag}(c_j)$; see Assumption 1 \\
\hline
...
\textbf{SVD of unit loading matrix} (equation (1))
...
\textbf{Prevalence-weighted factor covariances}
...
\textbf{Sample PCA objects} (equation (4))
...
\textbf{Limiting small Gram matrix} (Lemmas 7–8)
...
\textbf{Shrinkage and alignment} (Corollaries 1 and 5)
...
\textbf{Angles.} For unit vectors $u,v\in\mathbb{R}^p$, the angle between them satisfies $\cos\angle(u,v) = |\langle u,v\rangle|$ and $\sin^2\angle(u,v) = 1 - \langle u,v\rangle^2$.
...
\textbf{Assumption 1 (Per-column prevalence).} For each $j\in\{1,\ldots,k\}$,

\[
\frac{\Vert \beta_j(p)\Vert^2}{p} \;\longrightarrow\; c_j \;\in\; (0,\infty).
\]

\textbf{Assumption 2 (Gram convergence).} The normalized Gram matrix converges: $G(p)\to G_\infty$ for some positive definite $G_\infty\in\mathbb{R}^{k\times k}$.

\textbf{Assumption 3 (Spectral separation).} The matrix $M$ defined in (3) has $k$ distinct positive eigenvalues $m_1 > m_2 > \cdots > m_k > 0$. Under $G_\infty = I_k$ and diagonal $\Sigma_F = \mathrm{diag}(\sigma_1^2,\ldots,\sigma_k^2)$, we have $D = \mathrm{diag}(c_j\sigma_j^2)$ and Assumption 3 reduces to the strict ordering $c_1\sigma_1^2 > c_2\sigma_2^2 > \cdots > c_k\sigma_k^2 > 0$: no two factors contribute equally to cross-sectional variance.

\textit{Example 3.1.} In Example 2.1 with $c_1 = c_2 = 5$ and factor-return standard deviations $\sigma_1 = 0.10$, $\sigma_2 = 0.05$: $c_1\sigma_1^2 = 5\times 0.01 = 0.05 > c_2\sigma_2^2 = 5\times 0.0025 = 0.0125$. Assumption 3 is satisfied.

\textit{Remark (detection threshold).} Assumption 3 requires all eigenvalues $m_j > 0$. In the diagonal case this means $c_j\sigma_j^2 > 0$, which holds as long as each factor has positive variance and positive prevalence. The quantitative formula $\delta^2/(n\rho_j+\delta^2)$ applies when $\rho_j > 0$; this is the analogue of the BBP (Baik–Ben Arous–Péché) detection threshold in this regime. When a factor's signal strength falls below $\delta^2/n$, the corresponding sample eigenvector loses alignment with the population direction entirely, and the floor formula does not apply to that factor.

\textbf{Noise assumptions.} The entries of $Z$ are mean-zero, mutually independent within each column, with common variance $\delta^2 > 0$ and uniformly bounded $(2+\eta)$-th moment for some $\eta > 0$:
...
\textbf{Theorem (Multifactor Dispersion Bias).} \textit{Under Assumptions 1, 2, and 3 and the noise assumptions, conditional on $F$ and almost surely as $p\to\infty$:}

\textbf{Part (i): Probe-vector alignment.} \textit{(Requires only Assumption 1 and the noise assumptions.) For any deterministic sequence $v = v(p)\in\mathbb{R}^p$ with $|v|\le 1$,}

\[
H^\top v \;-\; H^\top\Pi_B v \;\longrightarrow\; 0 \quad a.s. \tag{5}
\]

\textbf{Part (ii): Per-direction alignment.} \textit{Under Assumptions 1–3, for each $j\in\{1,\ldots,k\}$,}
...
\textit{where $\rho_j$ and $\hat{w}_j$ are the $j$-th eigenvalue and eigenvector of $\hat{M}$, and $w_j$ is the $j$-th eigenvector of $M$.}

\textit{Specialization ($G_\infty = I_k$).} When $G_\infty = I_k$, Assumption 3 forces $M = D$ to be diagonal, so $w_j = e_j$ (the $j$-th standard basis vector of $\mathbb{R}^k$) and $\hat{M} = \hat{D}$, with $\rho_j$ the $j$-th eigenvalue of $\hat{D} = C^{1/2}(F^\top F/n)C^{1/2}$. Formula (6) then reads $\sin^2\angle(\hat{w}_j, w_j) = \sin^2\angle(\hat{w}_j, e_j)$.

The first term of (6) is the floor $1/(1+\mathrm{SNR}_j)$, determined entirely by the signal-to-noise ratio $\mathrm{SNR}_j = n\rho_j/\delta^2$. The second term is the in-subspace alignment error between finite-$n$ and population factor-covariance eigenvectors, scaled by $\mathrm{SNR}_j/(1+\mathrm{SNR}_j)$.

\subsection{Worked Examples}

\textbf{Example 4.1 ($k=1$, single factor).} Let $k=1$, $\beta = a\cdot\mathbf{1}_p$ for some $a > 0$, so $c = a^2$ and $G(p) = 1 = G_\infty$. The single factor-return series is $F = X\in\mathbb{R}^n$ with $\hat{D} = c\cdot\Vert X\Vert^2/n$ (a scalar), so $\rho = c\Vert X\Vert^2/n$ and $\hat{w}_1 = e_1 = 1$ trivially. The in-subspace rotation is $\sin^2\angle(1, 1) = 0$. Formula (6) (with $G_\infty = I_k$, so $w_j = e_j = 1$ trivially) reduces to
...
\textbf{Example 4.2 ($k=2$, diagonal case, orthogonal returns).} Use $k=2$ with the loading setup from Example 2.1: $c_1 = c_2 = 5$, $G_\infty = I_2$, and orthogonal factor returns ($F_1^\top F_2 = 0$). Take $n = 60$, $\sigma_1 = 0.10$, $\sigma_2 = 0.05$, $\delta = 1.0$.
...
The proof rests on two fundamental noise results. \textbf{Lemma 1} is the main probabilistic workhorse: a fourth-moment Borel–Cantelli argument showing that a single noise projection $\eta_p^\top Z_{\cdot\ell}/\sqrt{p}$ vanishes a.s. for any bounded deterministic sequence $\eta_p$. \textbf{Lemma 4} shows the noise Gram matrix $Z^\top Z/p$ concentrates around $\delta^2 I_n$, proved by the Kolmogorov SLLN. \textbf{Corollary 1.1} (following Lemma 1) extends the pointwise bound to the full matrix $U(p)^\top Z$, giving $\Vert U(p)^\top Z\Vert_F^2/p \to 0$ a.s. by applying Lemma 1 column-by-column.

\subsection{Lemma 1 (Bounded-vector noise concentration)}

\textit{Why needed.} Part (i)'s proof requires showing $\eta_p^\top Z_{\cdot\ell}/\sqrt{p}\to 0$ for a deterministic unit vector $\eta_p$. A second-moment Chebyshev bound fails; the fourth moment is required.

\textit{Let $\eta_p\in\mathbb{R}^p$ be a deterministic sequence with $|\eta_p|\le C$ uniformly in $p$. Under the noise assumptions, for each fixed $l\in\{1,\ldots,n\}$,}

\[
\frac{(\eta_p^\top Z)_l}{\sqrt{p}} \;\longrightarrow\; 0 \quad a.s.
\]

\textit{More precisely, $|(\eta_p^\top Z)_l| = o(p^{1/2-\varepsilon})$ a.s. for any fixed $\varepsilon\in(0,1/4)$.}

\textbf{Proof.} Fix $l$ and write $W_p = \sum_{i=1}^p a_i Z_{il}$ where $a_i = (\eta_p)_i$ and $\sum_i a_i^2 = |\eta_p|^2 \le C^2$.

\textit{Step 1: Fourth moment.} Expand $\mathbb{E}[W_p^4]$. By independence of $Z_{il}$ within column $l$ and mean-zero, only two index patterns survive: all-equal ($i_1 = i_2 = i_3 = i_4$) and two-pairs. Let $\kappa_4 = \sup_{i,p}\mathbb{E}[Z_{il}^4] < \infty$. Using $\sum_i a_i^4 \le C^4$ and $\sum_{i\ne j}a_i^2 a_j^2 \le C^4$:

\[
\mathbb{E}[W_p^4] \;\le\; C^4\kappa_4 + 3C^4\delta^4 \;=:\; K \;<\;\infty,\quad\text{uniformly in }p.
\]

\textit{Step 2: Markov.} $\Pr(|W_p| > p^{1/2-\varepsilon}) \le K/p^{2-4\varepsilon}.$

\textit{Step 3: Borel–Cantelli.} The series $\sum_p K/p^{2-4\varepsilon}$ converges for $\varepsilon < 1/4$. Borel–Cantelli gives $|W_p| \le p^{1/2-\varepsilon}$ for all large $p$ a.s., hence $W_p/\sqrt{p} \to 0$ a.s. $\square$

\textit{Remark.} A second-moment bound gives $\Pr(|W_p| > p^{1/2-\varepsilon}) \le C^2\delta^2/p^{1-2\varepsilon}$, summable only when $\varepsilon < 0$ — useless. The fourth moment is essential.

\medskip\noindent\rule{\linewidth}{0.4pt}\medskip

\subsection{Corollary 1.1 (Noise projection onto the signal subspace)}

\textit{Let $U(p)\in\mathbb{R}^{p\times k}$ be a deterministic sequence with orthonormal columns spanning $\mathcal{B}$. Then $\Vert U(p)^\top Z\Vert_F^2/p \to 0$ a.s. The same bound holds with $b(p)$ in place of $U(p)$: $\Vert b(p)^\top Z\Vert_F^2/p \to 0$ a.s.}

\textbf{Proof.} Each column $u_l(p)$ of $U(p)$ is a deterministic sequence of unit vectors. Lemma 1 applies to each pair $(l, m)$ with $l\in\{1,\ldots,k\}$, $m\in\{1,\ldots,n\}$:
...
\textit{Why Lemma 1 suffices (no Marcinkiewicz–Zygmund needed).} The elementary reason is that a rank-$k$ projector has bounded expected Frobenius norm: $\mathbb{E}\Vert \Pi_B Z\Vert_F^2 = \delta^2\mathrm{tr}(\Pi_B) = \delta^2 k$, independent of $p$. Lemma 1's fourth-moment Borel–Cantelli argument promotes this $O(1)$ expectation to almost-sure convergence of the normalized version $\Vert U^\top Z\Vert_F^2/p \to 0$.

\medskip\noindent\rule{\linewidth}{0.4pt}\medskip

\subsection{Lemma 4 (Noise Gram concentration)}

\textit{$Z^\top Z/p \to \delta^2 I_n$ a.s. in spectral norm.}

\textbf{Proof.} The $(j,\ell)$ entry of $Z^\top Z/p$ equals $(1/p)\sum_{i=1}^p Z_{ij}Z_{i\ell}$. For $j = \ell$ the summands $Z_{ij}^2$ are i.i.d. with mean $\delta^2$; for $j\ne\ell$ they have mean zero. The Kolmogorov SLLN gives entrywise a.s. convergence. Since $n$ is fixed, entrywise convergence implies spectral-norm convergence. $\square$

\medskip\noindent\rule{\linewidth}{0.4pt}\medskip

\section{Proof of Part (i)}

\textit{Proof sketch.} Project the PCA equation onto the noise subspace $\mathcal{B}^\perp$. The signal term $BF^\top$ is killed exactly (since $\Pi_B^\perp B = 0$). Only the noise term $Z$ survives, and Lemma 1 shows its contribution vanishes.

\textbf{Full proof.} Decompose $v = \Pi_B v + \eta_p$ where $\eta_p = \Pi_B^\perp v$. Since $\Pi_B^\perp$ is non-expansive, $|\eta_p|\le|v|\le 1$.
...
\textit{Note.} Part (i) requires only Assumption 1 and the noise assumptions. The projection $\Pi_B^\perp$ kills the signal identically — the argument is algebraic, independent of the loading correlation structure. The per-eigenvector results of Part (ii) additionally require Assumptions 2 and 3 to identify which part of $\mathcal{B}$ each sample eigenvector points toward.
...
\subsection{Parallel/Perpendicular Decomposition}
...
\[
\sin^2\angle(h_j,\,\bar{b}_j) \;=\; \Vert h_j^\perp\Vert^2 \;+\; \Vert h_j^{\parallel}\Vert^2\,\sin^2\!\angle\!\left(\frac{h_j^{\parallel}}{\Vert h_j^{\parallel}\Vert },\,\bar{b}_j\right). \tag{9}
\]
...
\subsection{Expansion of $W^{(p)}$ and Its Limit}
...
\textbf{Term (A).} $B^\top B/p = \Gamma_p \to \Gamma_B$ by Assumptions 1–2, giving
...
\textbf{Term (B).} Each column $b_j(p) = \beta_j/\Vert \beta_j\Vert $ of $b(p)$ is a deterministic unit vector. Corollary 1.1 gives $\Vert b(p)^\top Z\Vert_F^2/p \to 0$ a.s. Since $B^\top Z = A(p)\cdot b(p)^\top Z$ and $A(p)/\sqrt{p} \to C^{1/2}$ (bounded), $\Vert B^\top Z\Vert_F/(p\sqrt{n}) \to 0$ a.s., so term (B) vanishes in spectral norm.

\textbf{Term (C).} By Lemma 4, $(\mathrm{C}) \to (\delta^2/n)I_n$ a.s.
...
\subsection{Eigenstructure of $W_\infty$}

\textbf{Lemma 7} (Eigenstructure of $W_\infty$). \textit{The matrix $W_\infty = F\Gamma_B F^\top/n + (\delta^2/n)I_n$ has eigenvalues $\tau_j = \rho_j + \delta^2/n$ for $j\le k$ and $\delta^2/n$ for $j > k$, where $\rho_1 > \cdots > \rho_k > 0$ are the eigenvalues of $\hat{M} = \Lambda_G^{1/2}(Q^\top\hat{D}Q)\Lambda_G^{1/2}$. The top-$k$ eigenvectors are}
...
\textit{where $\hat{w}_j$ is the $j$-th eigenvector of $\hat{M}$ and $(F^\#)^\top = FC^{1/2}\in\mathbb{R}^{n\times k}$.} When $G_\infty = I_k$: $\hat{M} = \hat{D}$, $Q = I_k$, $\Lambda_G = I_k$, and (12) reduces to $v_j = (F^\#)^\top\hat{w}_j/\sqrt{n\rho_j}$.

\textbf{Proof.} Write $\Gamma_B = PP^\top$ with $P = C^{1/2}Q\Lambda_G^{1/2} \in \mathbb{R}^{k\times k}$, so $W_\infty - (\delta^2/n)I_n = (FP)(FP)^\top/n$ with $FP \in \mathbb{R}^{n\times k}$.

\textit{Step 1: Nonzero eigenvalues via AB/BA.} The nonzero eigenvalues of $(FP)(FP)^\top/n$ equal those of $(FP)^\top(FP)/n$ (AB/BA: if $ABx = \rho x$ with $\rho\ne 0$, then $BA(Ax) = \rho(Ax)$). Compute:
...
\textit{Step 2: Verify $v_j$ is an eigenvector.} Since $\hat{M}\hat{w}_j = \rho_j\hat{w}_j$, we have $(FP)^\top(FP)\hat{w}_j = n\rho_j\hat{w}_j$. Set $v_j = (FP)\hat{w}_j/\sqrt{n\rho_j}$. Then:

\[
\frac{(FP)(FP)^\top}{n}\,v_j \;=\; \frac{(FP)\,(FP)^\top(FP)\,\hat{w}_j}{n\sqrt{n\rho_j}} \;=\; \frac{(FP)\,n\rho_j\,\hat{w}_j}{n\sqrt{n\rho_j}} \;=\; \rho_j\,v_j.
\]

Expanding $FP = FC^{1/2}Q\Lambda_G^{1/2} = (F^\#)^\top Q\Lambda_G^{1/2}$ gives formula (12).

A key identity used in §7.6: from $\hat{M}\hat{w}_j = \rho_j\hat{w}_j$, left-multiplying successively by $\Lambda_G^{-1/2}$ and $Q$ gives:

\[
\hat{D}\,Q\Lambda_G^{1/2}\hat{w}_j \;=\; Q\,\rho_j\,\Lambda_G^{-1/2}\hat{w}_j. \tag{$*$}
\]

\textit{Step 3: Unit norm.} $\Vert v_j\Vert^2 = \hat{w}_j^\top(FP)^\top(FP)\hat{w}_j/(n\rho_j) = \hat{w}_j^\top\hat{M}\hat{w}_j/\rho_j = 1$.

\textit{Step 4: Bottom eigenvalues.} Any $u \perp \mathrm{col}(FP)$ satisfies $(FP)(FP)^\top u = 0$, so $W_\infty u = (\delta^2/n)u$. $\square$

\textit{Intuition.} $W_\infty$ is a rank-$k$ perturbation of the noise floor $(\delta^2/n)I_n$. The top-$k$ eigenvalues $\rho_j + \delta^2/n$ are signal-plus-noise; the remaining $n-k$ equal $\delta^2/n$. The gap $\rho_k > 0$ enables eigenvector convergence in step 7.4. The $\hat{M}$ eigenvalues $\rho_j$ encode the signal strengths under both the factor covariance $\hat{D}$ and the loading geometry $G_\infty$; when $G_\infty = I_k$ they reduce to eigenvalues of $\hat{D}$ directly.
...
\subsection{The $\Gamma_B$-Coordinate Framework}

\textit{The loading map.} Define $\Phi_p : \mathbb{R}^k \to \mathcal{B}$ by
...
\textit{Population direction.} Since $\bar{b}_j \in \mathcal{B}$ and $B$ has full column rank, write $\bar{b}_j = \Phi_p(a_j^{(p)})$ uniquely. Substituting into the eigenvalue equation $\Sigma_0^{(p)}\bar{b}_j = \lambda_j\bar{b}_j$ (with $\Sigma_0^{(p)} = B\Sigma_F B^\top/p$) and left-multiplying by $B^\top/\sqrt{p}$:
...
\textit{Sample direction.} From (4), the in-subspace component of $h_j$ is $\Pi_B h_j = \Pi_B Y\chi_{p,j}/(\sqrt{n}s_{p,j})$. Using $Y = BF^\top + Z$ and $\Pi_B B = B$:

\[
\Pi_B h_j \;=\; \frac{BF^\top\chi_{p,j}}{\sqrt{n}\,s_{p,j}} \;+\; \frac{\Pi_B Z\chi_{p,j}}{\sqrt{n}\,s_{p,j}}.
\]

For the noise term: each column $b_l = \beta_l/\Vert \beta_l\Vert $ of $b(p)$ is a deterministic unit vector, so Corollary 1.1 gives $\Vert b(p)^\top Z\Vert_F^2/p \to 0$ a.s., hence $\Vert \Pi_B Z\chi_{p,j}\Vert /\sqrt{p} \to 0$ a.s. Since $s_{p,j} \asymp \sqrt{p}$ a.s. (from §7.4), this noise term is $o(1)$ a.s.
...
\textit{Convergence of $g_j^{(p)}$.} Since $s_{p,j}^2/p \to \rho_j + \delta^2/n$ a.s. and $\chi_{p,j} \to v_j$ a.s. by (13), we have $\sqrt{p}/s_{p,j} \to 1/\sqrt{\rho_j + \delta^2/n}$ a.s. and $F^\top\chi_{p,j} \to F^\top v_j$. Hence:

\[
g_j^{(p)} \;\longrightarrow\; g_j^\infty \;:=\; \frac{F^\top v_j}{\sqrt{n\rho_j + \delta^2}} \quad\text{a.s.}
\]

This limit holds for the \textbf{full sequence}: every factor in (13) converges without subsequence extraction.

\subsection{Floor and In-Subspace Angle via $\Gamma_B$ Inner Products}
...
\textit{Step 1: Compute $g_j^\infty$ explicitly.} From (12), $v_j = FC^{1/2}Q\Lambda_G^{1/2}\hat{w}_j/\sqrt{n\rho_j}$. Apply $F^\top$ (dimension: $k\times n$) to get $F^\top v_j \in \mathbb{R}^k$:
...
\textit{Step 2: In-subspace squared norm and floor.} Compute $(g_j^\infty)^\top\Gamma_B g_j^\infty$. Since $\Gamma_B = C^{1/2}Q\Lambda_G Q^\top C^{1/2}$, the middle factor simplifies as $\Lambda_G^{-1/2}Q^\top C^{-1/2}\cdot C^{1/2}Q\Lambda_G Q^\top C^{1/2}\cdot C^{-1/2}Q\Lambda_G^{-1/2} = I_k$. Therefore:
...
\[
\Vert h_j^\perp\Vert^2 \;=\; 1 - \Vert \Pi_B h_j\Vert^2 \;\longrightarrow\; \frac{\delta^2}{n\rho_j+\delta^2} \quad\text{a.s.} \tag{15}
\]

\textit{Step 3: In-subspace inner product.} Recall $a_j^\infty = C^{-1/2}Q\Lambda_G^{-1/2}w_j$. The same simplification $\Lambda_G^{-1/2}Q^\top C^{-1/2}\cdot\Gamma_B\cdot C^{-1/2}Q\Lambda_G^{-1/2} = I_k$ gives:
...
\textit{Step 4: In-subspace angle.} Using (9) and the limits from Steps 2–3:
...
\textit{Diagonal-Gram specialization.} When $G_\infty = I_k$: $Q = I_k$, $\Lambda_G = I_k$, $w_j = e_j$ (since $M = D$ is diagonal under the ordering Assumption 3). Then $a_j^\infty = C^{-1/2}e_j = e_j/\sqrt{c_j}$, $\hat{w}_j^\top w_j = (\hat{w}_j)_j$, and $\sin^2\angle(\hat{w}_j, w_j) = \sin^2\angle(\hat{w}_j, e_j) = 1 - (\hat{w}_j)_j^2$, recovering the $G_\infty = I_k$ specialization stated after formula (6).
...
This is formula (6). All limits hold for the full sequence — no subsequence extraction is needed, because the $\Gamma_B$-coordinate framework operates directly on $\Gamma_p \to \Gamma_B$, bypassing the rotational ambiguity of $V(p)$ in the SVD basis. $\square$

\medskip\noindent\rule{\linewidth}{0.4pt}\medskip

\section{Remark on proof structure}

The proof of §7 treats $\Gamma_B = C^{1/2}G_\infty C^{1/2}$ as an arbitrary positive-definite matrix throughout — the $G_\infty = I_k$ specialization requires no separate argument. The $G_\infty = I_k$ case simply sets $\Gamma_B = C$, $\hat{M} = \hat{D}$, $M = D$, $w_j = e_j$, and $\sin^2\angle(\hat{w}_j, w_j) = \sin^2\angle(\hat{w}_j, e_j) = 1 - (\hat{w}_j)_j^2$.
...
\section{Recovery of NG's Theorem 3.1+}
...
\textit{Remark.} NG's Theorem 3.1+ Part (i) follows from Part (i) of the unified theorem with no additional assumptions. In particular, Part (i) of NG's Theorem 3.1+ does not require Assumptions 2.5+ or 2.6+.

\medskip\noindent\rule{\linewidth}{0.4pt}\medskip

\section{Corollaries}

\subsection{Corollary 1 (Out-of-subspace floor and its observable estimator)}

\textit{Under the assumptions of Part (ii), almost surely,}

\[
\liminf_{p\to\infty}\sin^2\angle(h_j,\bar{b}_j) \;\ge\; \frac{\delta^2}{n\rho_j+\delta^2}.
\]

\textit{This lower bound is observable: with $\ell_p^2 := \frac{1}{n-k}\sum_{j=k+1}^n s_{p,j}^2$ and}

\[
\hat{\psi}_{p,j}^2 \;:=\; 1 - \frac{\ell_p^2}{s_{p,j}^2},
\]

\textit{we have $\hat{\psi}_{p,j}^2 \xrightarrow{a.s.} n\rho_j/(n\rho_j+\delta^2) = 1 - \delta^2/(n\rho_j+\delta^2)$.}

\textbf{Proof.} The lower bound follows from (6) since the in-subspace term is non-negative. For the observable estimator: the noise singular values $s_{p,j}$ for $j > k$ satisfy $s_{p,j}^2/p\to\delta^2/n$ a.s. (they are eigenvalues of $W^{(p)}$ converging to $\delta^2/n$). Hence $\ell_p^2/p\to\delta^2/n$ and $s_{p,j}^2/p\to\rho_j+\delta^2/n$, giving $\hat{\psi}_{p,j}^2\to n\rho_j/(n\rho_j+\delta^2)$ a.s. $\square$

\textit{Note.} $\hat{\psi}_{p,j}^2$ requires only the sample singular values of $Y$ — no knowledge of $D$, $G_\infty$, $C$, or $Q$.

\subsection{Corollary 2 (GPS2022 recovery, $k=1$)}

\textit{Setting $k=1$: $B = \beta\in\mathbb{R}^p$, $F = X\in\mathbb{R}^{n\times 1}$, $c = \lim\Vert \beta\Vert^2/p$. Then}

\[
\sin^2\angle(h, \bar{b}) \;\to\; \frac{\delta^2}{c\Vert X\Vert^2 + \delta^2}.
\]

\textit{This recovers the main result of Goldberg–Papanicolaou–Shkolnik (2022).}

\textbf{Proof.} $\hat{D} = c\Vert X\Vert^2/n$ is a scalar, so $\rho_1 = c\Vert X\Vert^2/n$ and $\sin^2\angle(\hat{w}_1,e_1) = 0$. Substituting gives $\delta^2/(c\Vert X\Vert^2+\delta^2)$. $\square$

\subsection{Corollary 3 (Dispersion bias — NG's Theorem 3.1+ corollary)}

The \textit{dispersion bias} for portfolio $z$ is $|\Pi_B z|^2 - |\Pi_H z|^2$: the squared loading-subspace exposure minus the squared sample-eigenvector exposure.

\textit{Under the assumptions of §9, with $z = e/\sqrt{p}$ (the equal-weight portfolio) and $c_i^{\mathrm{ew}} := \langle b_i, z\rangle$:}

\[
|\Pi_B z|^2 - |\Pi_H z|^2 \;\to\; \sum_{i=1}^k (1-\psi_{\infty,i}^2)\,(c_i^{\mathrm{ew}})^2 \;>\; 0 \quad a.s.
\]

\textbf{Proof.} From Part (i) at $v = z$: $|H^\top z|^2\to|H^\top\Pi_B z|^2$. From §9, $H^\top\tilde{B}\to\mathrm{diag}(\psi_{\infty,i})$ and $\langle b_i, z\rangle \to c_i^{\mathrm{ew}}$. Hence $|H^\top\Pi_B z|^2\to\sum_i\psi_{\infty,i}^2 (c_i^{\mathrm{ew}})^2$ and $|\Pi_B z|^2\to\sum_i(c_i^{\mathrm{ew}})^2$. The deficit $\sum_i(1-\psi_{\infty,i}^2)(c_i^{\mathrm{ew}})^2 > 0$ since each $\psi_{\infty,i}^2 < 1$ and $c_i^{\mathrm{ew}} > 0$. $\square$

\textit{Example.} Using Example 4.2: $(1-0.750)(0.8) + (1-0.429)(0.2) = 0.200 + 0.114 = 0.314$.

\subsection{Corollary 4 (Grassmannian subspace distance)}
...
\textit{where $\theta_1, \ldots, \theta_k$ are the principal angles between $\mathrm{col}(H)$ and $\mathcal{B}$.}

\textbf{Proof.} The squared chordal distance on $\mathrm{Gr}(k,p)$ satisfies the identity
...
The limit now follows directly from equation (15): $\Vert h_j^\perp\Vert^2\to\delta^2/(n\rho_j+\delta^2)$ a.s. for each $j$, and $k$ is fixed. $\square$

\textit{Remark.} The in-subspace rotation term in the per-vector formula (6) does not appear in Corollary 4. This is not a coincidence: the Grassmannian distance depends only on $\Pi_H = HH^\top$, which is invariant under orthogonal rotations of the columns of $H$ within $\mathrm{col}(H)$. The in-subspace rotation is precisely such a rotation — it displaces each $h_j$ within $\mathcal{B}$ without changing the subspace $\mathrm{col}(H)$. Corollary 4 therefore provides a strictly weaker error criterion than the per-vector formulas, and achieves a correspondingly smaller limiting error.

\medskip\noindent\rule{\linewidth}{0.4pt}\medskip

\subsection{Corollary 5 (Frame-Level Dispersion Bias)}

\textbf{Setup.} Let $W \in \mathbb{R}^{p \times k_W}$ be a deterministic probe frame with $1 \le k_W \le n$ orthonormal columns ($W^\top W = I_{k_W}$). Assume the asymptotic alignment matrix
...
\textit{Note on notation.} $\Gamma_\infty$ is a new $k \times k_W$ matrix; it does not conflict with $C = \mathrm{diag}(c_j)$, the diagonal prevalence matrix from §2. The shorthand $\Psi_\infty := \mathrm{diag}(\psi_{\infty,1},\ldots,\psi_{\infty,k})$ is used throughout; recall $\psi_{\infty,j}^2 = n\rho_j/(n\rho_j+\delta^2)$.

\textbf{Part (i): Subspace alignment of probe frame.} \textit{Under Assumption 1 and the noise assumptions only (no orthogonality conditions on $B$ or $F$), and requiring only $\Vert W\Vert_{\mathrm{op}} \le 1$:}

\[
H^\top W \;-\; H^\top \Pi_B W \;\longrightarrow\; 0 \quad \text{a.s.}
\]

\textit{in any matrix norm.}

\textbf{Part (ii): Frame factorization, NG case.} \textit{Under the additional hypotheses $G_\infty = I_k$ and orthogonal factor returns (the setting of §9, requiring NG's Assumptions 2.5' and 2.6'):}

\[
H^\top W \;\longrightarrow\; \Psi_\infty\, \Gamma_\infty \quad \text{a.s.}
\]

\textbf{Four consequences follow.}

\textit{(a) Frame-level Frobenius deficit.}
...
\textit{(b) Principal angle shrinkage.} The cosines of the principal angles between $\mathrm{span}(W)$ and $\mathrm{span}(H)$ are asymptotically the singular values of $\Psi_\infty \Gamma_\infty$:
...
\textit{(c) Per-factor decomposition.} Writing $(\Gamma_\infty\Gamma_\infty^\top)_{jj} = \sum_l (\gamma_j^{(l)})^2$ for the total exposure of probe frame $W$ to factor $j$:
...
\textit{(d) Probing the population frame.} When $k_W = k$ and $W = \tilde{B}$ (the population loading frame itself), $\Gamma_\infty = I_k$ and $(\Gamma_\infty\Gamma_\infty^\top)_{jj} = 1$, giving
...
\textit{Part (i).} Apply Part (i) of the unified theorem to each column $w_l$ of $W$ with bound $|w_l| \le \Vert W\Vert_{\mathrm{op}} \le 1$. Each column gives $H^\top w_l - H^\top\Pi_B w_l \to 0$ a.s. The finite intersection of $k_W$ probability-one events has probability one. Assembling column-wise gives the matrix statement; convergence in any matrix norm follows since $H^\top W$ has fixed dimensions $k \times k_W$.

\textit{Part (ii).} From Part (i), $H^\top W = H^\top\Pi_B W + o(1)$ a.s. Under $G_\infty = I_k$ (NG's Assumption 2.5'), the unit loading columns satisfy $G(p) = b(p)^\top b(p) = I_k$ exactly, so $\Pi_B = \tilde{B}\tilde{B}^\top$ and
...
\subsection{The role of loading geometry}

Formula (6) covers both cases: when $G_\infty = I_k$, $\hat{M} = \hat{D}$, $M = D$, and $w_j = e_j$; for general $G_\infty = Q\Lambda_G Q^\top$, the loading geometry rotates and reweights the factor covariance via $\hat{M}$ and $M$. In practice, fundamental factor models (Barra, Axioma) commonly produce non-orthogonal loading columns, making the general form of (6) the operationally relevant one.
...
\hline
General $G_\infty$ & Full formula (6) with $\hat{M}$, $M$ \\
\hline
...
\section{Grassmannian Subspace Estimation vs. Frame Estimation}

\subsection{Two Estimation Targets}

The theorem supports two estimation goals with different error criteria.

\textit{Frame estimation} aims to recover the matrix $[\bar{b}_1, \ldots, \bar{b}_k]$ as a specific $k$-frame in $\mathbb{R}^p$. The sample estimator is $H = [h_1, \ldots, h_k]$. The total frame error is

\[
\mathcal{E}_{\mathrm{frame}} \;:=\; \sum_{j=1}^k \sin^2\angle(h_j,\, \bar{b}_j).
\]

\textit{Subspace estimation} aims to recover $\mathcal{B} = \mathrm{col}(B)$ as a point on the Grassmannian $\mathrm{Gr}(k,p)$. The sample estimator is $\mathrm{col}(H)$ — the same linear span, treated as an equivalence class under orthogonal rotation. The total subspace error is the squared chordal distance $d_{\mathrm{Gr}}^2(\mathrm{col}(H), \mathcal{B}) = \sum_j \sin^2\theta_j$.
...
\subsection{The Floor-Plus-Rotation Decomposition Revisited}
...
\subsection{When Does the Rotation Term Contribute?}
...
\subsection{Subspace Estimation Dominates Frame Estimation}
...
\subsection{Practical Implications for Factor Modeling}

Two concrete adjustments follow from these results.

For \textit{subspace-based tasks} (factor neutralization, risk decomposition, portfolio projection), use $\mathrm{col}(H)$ directly. The projection $\Pi_H z \approx \Pi_B z$ for any portfolio $z$ by Part (i) of the theorem, and the Grassmannian error quantifies how close this approximation is.

For \textit{frame-based tasks} (estimating individual factor loadings, computing factor exposures for attribution), the shrinkage estimator $H \cdot \mathrm{diag}(\hat{\psi}_{p,j})$ (with $\hat{\psi}_{p,j}$ from Corollary 1) reduces each eigenvector toward the origin by the appropriate signal-to-noise scaling, partially compensating for the out-of-subspace floor. This does not eliminate the rotation excess $\mathcal{E}_{\mathrm{rotation}}$, which requires either longer time series or a factor model that enforces orthogonal returns.
...
\textit{First}, the out-of-subspace floor $\delta^2/(n\rho_j + \delta^2)$ is irreducible in the cross-section: adding more assets (larger $p$) does not reduce it, because the signal-to-noise ratio is determined by $n$ alone. The unique lever is a longer time series.
