...
*This document consolidates two complementary lines of work: the k-factor dispersion bias result and the multifactor prevalence extension. The result is a single, fully proved theorem that subsumes both. Term definitions appear in §2; the introduction uses terms informally*
...
**Analytical strategy.** The theorem measures $\sin^2\angle(h_j, \bar{b}_j)$ for each $j \in \{1,\ldots,k\}$, where $h_j \in \mathbb{R}^p$ is the $j$-th sample principal component of $Y$ and $\bar{b}_j \in \mathbb{R}^p$ is the $j$-th population loading direction — the $j$-th eigenvector of the population signal covariance $B\Sigma_F B^\top / p$ (defined precisely in the *Population loading direction* paragraph below). The central analytical device is to replace the $p\times p$ matrix $YY^\top/n$ — whose spectrum grows without bound as $p\to\infty$ — with the $n\times n$ *dual* matrix

$$
W^{(p)} = \frac{Y^\top Y}{np} \in \mathbb{R}^{n\times n}.
$$

The two matrices share the same nonzero eigenvalues (up to the factor $p$), so the full PCA geometry of $YY^\top/n$ is captured by the fixed-size $W^{(p)}$. The factor component of $W^{(p)}$ is $FB^\top BF^\top/(np)$; by the AB/BA eigenvalue identity, its nonzero eigenvalues equal those of $B^\top BF^\top F/(np)$, which is a $k\times k$ matrix involving the Gram $B^\top B/p$ and the factor-return covariance $F^\top F/n$. As $p\to\infty$, $W^{(p)}$ converges almost surely to a deterministic $n\times n$ limit $W_\infty$ (established in Appendix B.3 (steps B.3.2–B.3.3)), and all the theorem+s quantities are read off from the eigenstructure of $W_\infty$. The remainder of this section defines the objects that appear in $W_\infty$.

**Prevalence and normalized loadings.** Write $A(p) = \mathrm{diag}(\Vert \beta_j(p)\Vert )_{j=1}^k$ for the diagonal matrix of loading-column norms, and
...
The *prevalence* of factor $j$ is $c_j = \lim_{p\to\infty}\Vert \beta_j(p)\Vert^2/p \in (0,\infty)$, and $C = \mathrm{diag}(c_1,\ldots,c_k)$.

*Intuition.* The prevalence $c_j$ measures how strongly factor $j$ is spread across the asset universe. If every asset has loading $a_j$ on factor $j$ (a uniform loading column $\beta_j = a_j\cdot\mathbf{1}_p$), then $\Vert \beta_j\Vert^2 = p\,a_j^2$, so $c_j = a_j^2$. If factor $j$ affects only a fixed finite subset of assets, $c_j = 0$ — the factor has zero prevalence and effectively disappears in the large-$p$ limit. The theory requires $c_j > 0$: each factor must be pervasive across assets.
...
Then $\Vert \beta_1\Vert^2 = p(9+1)/2 = 5p$, so $c_1 = 5$; likewise $c_2 = 5$. Since $\beta_1^\top\beta_2 = p(3)(-1)/2 + p(1)(3)/2 = 0$, the loading columns are orthogonal, so $G(p) = I_2$ and $G_\infty = I_2$. We return to this example numerically in §4.1.
...
$$
\hat{D} = C^{1/2}\!\left(\frac{F^\top F}{n}\right)C^{1/2}, \qquad D = C^{1/2}\Sigma_F C^{1/2},
$$

where $\Sigma_F = \lim_{n\to\infty} F^\top F/n$ is the population factor-return covariance. The hat on $\hat{D}$ signals dependence on the finite time series through $F^\top F/n$ rather than on its limit $\Sigma_F$; as $n\to\infty$, $\hat{D} \to D$. Neither $C$ nor $F$ is estimable from the observed data $Y$ alone: $C$ requires the large-$p$ limit of loading norms, and $F$ is conditioned on throughout. Equivalently, $\hat{D} = F^\#(F^\#)^\top/n$.
...
*Remark on loading norms.* The diagonal matrix $A(p) = \mathrm{diag}(\Vert \beta_j(p)\Vert )$ satisfies $A(p)/\sqrt{p} \to C^{1/2}$ by Assumption 1. Equivalently, $B^\top B/p = A(p)G(p)A(p)/p \to C^{1/2}G_\infty C^{1/2} = \Gamma_B$. This convergence $\Gamma_p \to \Gamma_B$ is all that the proof of Appendix B.3 requires; the matrices $A(p)$ and $G(p)$ enter only through their product.

**Rotated-and-reweighted matrices for general $G_\infty$.** Fix a spectral decomposition

$$
G_\infty = Q\Lambda_G Q^\top, \qquad \Lambda_G = \mathrm{diag}(g_1,\ldots,g_k),\quad g_1\ge\cdots\ge g_k>0,\quad Q\in O(k).\\ \tag{1}
$$

Define

$$
\hat{M} = \Lambda_G^{1/2}(Q^\top \hat{D}\,Q)\Lambda_G^{1/2}, \qquad M = \Lambda_G^{1/2}(Q^\top D\,Q)\Lambda_G^{1/2}. \tag{2}
$$
...
**Eigenvalues and eigenvectors of $\hat{M}$ and $M$.** Write $\rho_1 > \cdots > \rho_k > 0$ for the eigenvalues of $\hat{M}$, with corresponding orthonormal eigenvectors $\hat{w}_1, \ldots, \hat{w}_k$. Write $w_1, \ldots, w_k$ for the orthonormal eigenvectors of $M$ and $d_j = \lim_{n\to\infty}\rho_j$ for the $j$-th population eigenvalue. The *signal-to-noise ratio* of factor $j$ is $\mathrm{SNR}_j = n\rho_j/\delta^2$; the corresponding *out-of-subspace floor* is $\delta^2/(n\rho_j + \delta^2) = 1/(1 + \mathrm{SNR}_j)$.

**Sample PCA objects.** The thin SVD of $Y/\sqrt{n}$ gives

$$
H S_p = \frac{Y\,\mathcal{X}_p}{\sqrt{n}}, \tag{3}
$$
...
The orthogonal projection onto $\mathrm{col}(H)$ is $\Pi_H = HH^\top \in \mathbb{R}^{p\times p}$.
...
corresponding to its $j$-th largest eigenvalue. Since $\Sigma_0^{(p)}$ is supported on $\mathcal{B}$, we have $\bar{b}_j\in\mathrm{col}(U(p)) = \mathcal{B}$. When the loading columns are mutually orthogonal, $\bar{b}_j$ coincides with the $j$-th normalized loading column $\beta_j/\Vert \beta_j\Vert $.

**Symbol reference.** All symbols introduced in this section are collected with their definitions in Appendix A. Symbols arising first in the proofs ($W_\infty$, $\tau_j$, $v_j$) are defined in Appendix B.1 and also listed in Appendix A. The shrinkage quantities ($\psi_{\infty,j}^2$, $\hat{\psi}_{p,j}^2$, $\ell_p^2$, $\Psi_\infty$) are defined in §5 (Corollaries 1 and 5) and also listed in Appendix A.
...
$$
\frac{\Vert \beta_j(p)\Vert^2}{p} \;\longrightarrow\; c_j \;\in\; (0,\infty).
$$
...
*Remark (assumption hierarchy for the two parts of the theorem).* **Part (i)** requires only Assumption 1 and the noise assumptions — no Gram convergence, no spectral separation, and in particular no orthogonality condition on the loading columns or factor returns. **Part (ii)** requires the full set of Assumptions 1–3.

---

## 4. Main Results

The theorem has two parts with distinct assumption requirements. Part (i) is a qualitative subspace alignment result, valid under minimal assumptions. Part (ii) is a quantitative per-direction result requiring the full assumption set; it gives the explicit two-term floor-plus-rotation decomposition for general positive-definite $G_\infty$.
...
$$
H^\top v \;-\; H^\top\Pi_B v \;\longrightarrow\; 0 \quad a.s. \tag{4}
$$

**Part (ii): Per-direction alignment.** *Under Assumptions 1–3, for each $j\in\{1,\ldots,k\}$,*

$$
\sin^2\angle(h_j,\,\bar{b}_j) \;\xrightarrow{a.s.}\;
\underbrace{\frac{\delta^2}{n\rho_j + \delta^2}}_{\text{out-of-subspace floor}}
\;+\;
\underbrace{\frac{n\rho_j}{n\rho_j + \delta^2}\,\sin^2\angle(\hat{w}_j,\,w_j)}_{\text{in-subspace rotation}},
\tag{5}
$$

*where $\rho_j$ and $\hat{w}_j$ are the $j$-th eigenvalue and eigenvector of $\hat{M}$, and $w_j$ is the $j$-th eigenvector of $M$.*

*Specialization ($G_\infty = I_k$).* When $G_\infty = I_k$, Assumption 3 forces $M = D$ to be diagonal, so $w_j = e_j$ (the $j$-th standard basis vector of $\mathbb{R}^k$) and $\hat{M} = \hat{D}$, with $\rho_j$ the $j$-th eigenvalue of $\hat{D} = C^{1/2}(F^\top F/n)C^{1/2}$. Formula (5) then reads $\sin^2\angle(\hat{w}_j, w_j) = \sin^2\angle(\hat{w}_j, e_j)$.

The first term of (5) is the floor $1/(1+\mathrm{SNR}_j)$, determined entirely by the signal-to-noise ratio $\mathrm{SNR}_j = n\rho_j/\delta^2$. The second term is the in-subspace alignment error between finite-$n$ and population factor-covariance eigenvectors, scaled by $\mathrm{SNR}_j/(1+\mathrm{SNR}_j)$.

*Proofs.* The proof of Part (i) is in Appendix B.2; the proof of Part (ii) is in Appendix B.3. The recovery of Theorem 3.1+ under the orthogonality hypotheses is in Appendix B.5.

### 4.1 Worked Examples

**Example 4.1 ($k=1$, single factor).** Let $k=1$, $\beta = a\cdot\mathbf{1}_p$ for some $a > 0$, so $c = a^2$ and $G(p) = 1 = G_\infty$. The single factor-return series is $F = X\in\mathbb{R}^n$ with $\hat{D} = c\cdot\Vert X\Vert^2/n$ (a scalar), so $\rho = c\Vert X\Vert^2/n$ and $\hat{w}_1 = e_1 = 1$ trivially. The in-subspace rotation is $\sin^2\angle(1, 1) = 0$. Formula (5) (with $G_\infty = I_k$, so $w_j = e_j = 1$ trivially) reduces to
...
*Under the assumptions of Part (ii), almost surely,*
...
*Setting $k=1$: $B = \beta\in\mathbb{R}^p$, $F = X\in\mathbb{R}^{n\times 1}$, $c = \lim\Vert \beta\Vert^2/p$. Then*

$$
\sin^2\angle(h, \bar{b}) \;\to\; \frac{\delta^2}{c\Vert X\Vert^2 + \delta^2}.
$$

*This recovers the main result of Goldberg–Papanicolaou–Shkolnik (2022).*

**Proof.** $\hat{D} = c\Vert X\Vert^2/n$ is a scalar, so $\rho_1 = c\Vert X\Vert^2/n$ and $\sin^2\angle(\hat{w}_1,e_1) = 0$. Substituting gives $\delta^2/(c\Vert X\Vert^2+\delta^2)$. $\square$

### Corollary 3 (Dispersion bias)
...
*Under the assumptions of Appendix B.5, with $z = e/\sqrt{p}$ (the equal-weight portfolio) and $c_i^{\mathrm{ew}} := \langle b_i, z\rangle$:*

$$
|\Pi_B z|^2 - |\Pi_H z|^2 \;\to\; \sum_{i=1}^k (1-\psi_{\infty,i}^2)\,(c_i^{\mathrm{ew}})^2 \;>\; 0 \quad a.s.
$$

**Proof.** From Part (i) at $v = z$: $|H^\top z|^2\to|H^\top\Pi_B z|^2$. From Appendix B.5, $H^\top\tilde{B}\to\mathrm{diag}(\psi_{\infty,i})$ and $\langle b_i, z\rangle \to c_i^{\mathrm{ew}}$. Hence $|H^\top\Pi_B z|^2\to\sum_i\psi_{\infty,i}^2 (c_i^{\mathrm{ew}})^2$ and $|\Pi_B z|^2\to\sum_i(c_i^{\mathrm{ew}})^2$. The deficit $\sum_i(1-\psi_{\infty,i}^2)(c_i^{\mathrm{ew}})^2 > 0$ since each $\psi_{\infty,i}^2 < 1$ and $c_i^{\mathrm{ew}} > 0$. $\square$
...
$$
\sum_{j=1}^k \sin^2\theta_j \;=\; \sum_{j=1}^k \Vert h_j^\perp\Vert^2,
$$

where $h_j^\perp = (I - \Pi_B)h_j$ is the out-of-subspace component of $h_j$. To see this: the principal angles are defined by the singular values of $H^\top U$ (where $U$ spans $\mathcal{B}$), with $\cos\theta_j = \sigma_j(H^\top U)$. Hence $\sum_j\cos^2\theta_j = \Vert H^\top U\Vert_F^2 = \mathrm{tr}(H^\top\Pi_B H) = \Vert \Pi_B H\Vert_F^2 = \sum_j\Vert h_j^{\parallel}\Vert^2$. Since $\Vert h_j^\perp\Vert^2 + \Vert h_j^{\parallel}\Vert^2 = 1$, summing gives $\sum_j\sin^2\theta_j = \sum_j\Vert h_j^\perp\Vert^2$.

The limit now follows directly from equation (13): $\Vert h_j^\perp\Vert^2\to\delta^2/(n\rho_j+\delta^2)$ a.s. for each $j$, and $k$ is fixed. $\square$

*Remark.* The in-subspace rotation term in the per-vector formula (5) does not appear in Corollary 4. This is not a coincidence: the Grassmannian distance depends only on $\Pi_H = HH^\top$, which is invariant under orthogonal rotations of the columns of $H$ within $\mathrm{col}(H)$. The in-subspace rotation is precisely such a rotation — it displaces each $h_j$ within $\mathcal{B}$ without changing the subspace $\mathrm{col}(H)$. Corollary 4 therefore provides a strictly weaker error criterion than the per-vector formulas, and achieves a correspondingly smaller limiting error.
...
exists, where $\tilde{B} = b(p)$ is the matrix of unit loading columns. (Sufficient condition: $\langle b_j, w_l\rangle \to \gamma_j^{(l)}$ for each $j$, $l$, in which case $(\Gamma_\infty)_{jl} = \gamma_j^{(l)}$.) Part (i) below holds under the weaker condition $\Vert W\Vert_{\mathrm{op}} \le 1$ without requiring $W^\top W = I_{k_W}$.
...
**Part (i): Subspace alignment of probe frame.** *Under Assumption 1 and the noise assumptions only (no orthogonality conditions on $B$ or $F$), and requiring only $\Vert W\Vert_{\mathrm{op}} \le 1$:*
...
**Part (ii): Frame factorization.** *Under the additional hypotheses $G_\infty = I_k$ and orthogonal factor returns (the setting of Appendix B.5):*
...
$$
\Vert \Pi_B W\Vert_F^2 \;-\; \Vert \Pi_H W\Vert_F^2 \;\longrightarrow\; \sum_{j=1}^k \frac{\delta^2}{n\rho_j + \delta^2}\,(\Gamma_\infty\Gamma_\infty^\top)_{jj} \;\ge\; 0 \quad \text{a.s.,}
$$
...
$$
\Vert \Pi_B W\Vert_F^2 - \Vert \Pi_H W\Vert_F^2 \;\longrightarrow\; \sum_{j=1}^k \underbrace{\frac{\delta^2}{n\rho_j + \delta^2}}_{\text{floor for factor } j} \cdot \underbrace{\sum_{l=1}^{k_W} (\gamma_j^{(l)})^2}_{\text{frame exposure to factor }j}.
$$
...
$$
\Vert \Pi_B \tilde{B}\Vert_F^2 - \Vert \Pi_H \tilde{B}\Vert_F^2 \;\longrightarrow\; \sum_{j=1}^k \frac{\delta^2}{n\rho_j + \delta^2} \;=\; d_{\mathrm{Gr}}^2(\mathrm{col}(H),\,\mathcal{B}).
$$
...
*Part (i).* Apply Part (i) of the unified theorem to each column $w_l$ of $W$ with bound $|w_l| \le \Vert W\Vert_{\mathrm{op}} \le 1$. Each column gives $H^\top w_l - H^\top\Pi_B w_l \to 0$ a.s. The finite intersection of $k_W$ probability-one events has probability one. Assembling column-wise gives the matrix statement; convergence in any matrix norm follows since $H^\top W$ has fixed dimensions $k \times k_W$.

*Part (ii).* From Part (i), $H^\top W = H^\top\Pi_B W + o(1)$ a.s. Under $G_\infty = I_k$, the unit loading columns satisfy $G(p) = b(p)^\top b(p) = I_k$ exactly, so $\Pi_B = \tilde{B}\tilde{B}^\top$ and

$$
H^\top\Pi_B W \;=\; (H^\top\tilde{B})(\tilde{B}^\top W).
$$

By Appendix B.5 (which additionally requires orthogonal factor returns, Assumption 2.6'), $H^\top\tilde{B} \to \Psi_\infty$ a.s. By the corollary's hypothesis, $\tilde{B}^\top W \to \Gamma_\infty$. Continuity of matrix multiplication gives $H^\top W \to \Psi_\infty\Gamma_\infty$ a.s.

*Consequences (a)–(d).* For (a): $\Vert \Pi_B W\Vert_F^2 = \Vert \tilde{B}^\top W\Vert_F^2 \to \Vert \Gamma_\infty\Vert_F^2 = \mathrm{tr}(\Gamma_\infty\Gamma_\infty^\top)$ and $\Vert \Pi_H W\Vert_F^2 = \Vert H^\top W\Vert_F^2 \to \Vert \Psi_\infty\Gamma_\infty\Vert_F^2 = \mathrm{tr}(\Gamma_\infty^\top\Psi_\infty^2\Gamma_\infty) = \sum_j\psi_{\infty,j}^2(\Gamma_\infty\Gamma_\infty^\top)_{jj}$. Subtracting gives (a) with $1 - \psi_{\infty,j}^2 = \delta^2/(n\rho_j+\delta^2)$. Parts (b)–(c) follow by continuity of singular values and direct expansion. Part (d) substitutes $\Gamma_\infty = I_k$ and invokes Corollary 4. $\square$
...
*Mixed-probe shrinkage is non-linear.* For a single probe ($k_W = 1$) with equal exposure to both factors — $\Gamma_\infty = (1,\ldots,1)^\top/\sqrt{k}$ — the sample principal cosine is $\Vert \Psi_\infty\Gamma_\infty\Vert  = (\sum_j\psi_{\infty,j}^2/k)^{1/2}$, the root-mean-square of the individual shrinkages. For $k=2$ with $\psi_{\infty,1} = 0.9$ and $\psi_{\infty,2} = 0.5$: $\Vert \Psi_\infty\Gamma_\infty\Vert  = \sqrt{(0.81+0.25)/2} \approx 0.728$, while the population cosine is $1$. This is strictly less than the arithmetic mean $(0.9+0.5)/2 = 0.7$ — the mixed probe suffers more than naive averaging would suggest. In contrast, for an orthogonal square probe frame ($k_W = k$, $\Gamma_\infty \in O(k)$), the singular values of $\Psi_\infty\Gamma_\infty$ are always exactly $\{\psi_{\infty,1},\ldots,\psi_{\infty,k}\}$, since $(\Psi_\infty\Gamma_\infty)(\Psi_\infty\Gamma_\infty)^\top = \Psi_\infty^2$. The non-linearity is a feature of mixed, non-orthogonal, or rectangular $\Gamma_\infty$.
...
Formula (5) covers both cases: when $G_\infty = I_k$, $\hat{M} = \hat{D}$, $M = D$, and $w_j = e_j$; for general $G_\infty = Q\Lambda_G Q^\top$, the loading geometry rotates and reweights the factor covariance via $\hat{M}$ and $M$. In practice, fundamental factor models (Barra, Axioma) commonly produce non-orthogonal loading columns, making the general form of (5) the operationally relevant one.

### Specializations

| Setting                         | Formula simplifies to                                                           |
|:------------------------------- |:------------------------------------------------------------------------------- |
| $k=1$ (GPS2022)                 | $\sin^2\angle(h,\bar{b}) \to \delta^2/(c\Vert X\Vert^2+\delta^2)$ (Corollary 2) |
| $G_\infty=I_k$, orth. returns   | $\sin^2\angle(h_j,\bar{b}_j) \to \delta^2/(n\rho_j+\delta^2)$; floor only       |
| $G_\infty=I_k$, general returns | Formula (5) with rotation $\sin^2\angle(\hat{w}_j,e_j)$                         |
| General $G_\infty$              | Full formula (5) with $\hat{M}$, $M$                                            |
...
### 7.2 The Floor-Plus-Rotation Decomposition Revisited
...
### 7.3 When Does the Rotation Term Contribute?

The rotation excess $\mathcal{E}_{\mathrm{rotation}}$ is zero if and only if $\sin^2\angle(\hat{w}_j, w_j) = 0$ for all $j$. Under Assumption 3, this holds if and only if $\hat{M} = M$ (sample and population factor covariance matrices coincide, up to an eigenvalue ordering), which is enforced exactly by orthogonal factor returns. In all other cases — correlated factor returns, or non-orthogonal loading columns with general $G_\infty$ — the rotation excess is strictly positive, and frame estimation incurs a penalty not present in subspace estimation.
...
### 7.4 Subspace Estimation Dominates Frame Estimation
...
For fixed $n$, the Grassmannian error $\mathcal{E}_{\mathrm{subspace}} = \sum_j \delta^2/(n\rho_j + \delta^2)$ is the minimum achievable subspace error: the estimator $\mathrm{col}(H)$ attains it, and the identity $\sum_j\sin^2\theta_j = \sum_j\Vert h_j^\perp\Vert^2$ shows that this equals the total out-of-subspace squared norm, which is determined entirely by the signal-to-noise structure and cannot be reduced by any choice of estimator for the subspace.

### 7.6 Practical Implications for Factor Modeling
...
*This appendix collects all symbols used in the paper. For symbols defined in the main text the table gives the location; the definition is not duplicated here. For symbols defined only in the appendices or corollaries the full definition is retained.*

**Model primitives**

| Symbol                                    | Defined                                                                                          |
|:-----------------------------------------:|:------------------------------------------------------------------------------------------------ |
| $p$, $n$, $k$                             | → §2 (Data model): number of assets, time periods, factors; $p\to\infty$ with $n$, $k$ fixed     |
| $Y \in \mathbb{R}^{p\times n}$            | → §2 (Data model): $Y = BF^\top + Z$                                                             |
| $B \in \mathbb{R}^{p\times k}$, $\beta_j$ | → §2 (Data model): loading matrix; $j$-th loading column                                         |
| $F \in \mathbb{R}^{n\times k}$, $F_j$     | → §2 (Data model): factor-return matrix; $j$-th factor-return series                             |
| $Z \in \mathbb{R}^{p\times n}$            | → §2 (Data model): idiosyncratic noise matrix                                                    |
| $\delta^2 > 0$                            | → §3 (Noise assumptions): common idiosyncratic noise variance                                    |
| $\Sigma_F$                                | → §2 (Prevalence-weighted factor covariance matrices): $\Sigma_F = \lim_{n\to\infty} F^\top F/n$ |

**Loading geometry**

| Symbol                               | Defined                                                                                                      |
|:------------------------------------:|:------------------------------------------------------------------------------------------------------------ |
| $A(p)$                               | → §2 (Prevalence and normalized loadings): $A(p) = \mathrm{diag}(\Vert \beta_j(p)\Vert )_{j=1}^k$            |
| $b(p) = BA(p)^{-1}$                  | → §2 (Prevalence and normalized loadings): unit-normalized loading matrix                                    |
| $G(p) = b(p)^\top b(p)$              | → §2 (Prevalence and normalized loadings): Gram matrix; $G(p) \to G_\infty$ by Assumption 2                  |
| $G_\infty$                           | → §2 (Prevalence and normalized loadings), Assumption 2                                                      |
| $Q$, $\Lambda_G$                     | → §2, equation (1): $G_\infty = Q\Lambda_G Q^\top$, $\Lambda_G = \mathrm{diag}(g_1,\ldots,g_k)$, $Q\in O(k)$ |
| $c_j$, $C$                           | → §2 (Prevalence and normalized loadings): $c_j = \lim\Vert \beta_j\Vert^2/p$; $C = \mathrm{diag}(c_j)$      |
| $\Gamma_B = C^{1/2}G_\infty C^{1/2}$ | → §2 (Remark on loading norms): $B^\top B/p \to \Gamma_B$                                                    |

**Signal subspace**

| Symbol                                | Defined                                                                                           |
|:-------------------------------------:|:------------------------------------------------------------------------------------------------- |
| $\mathcal{B} = \mathrm{col}(B)$       | → §2 (Signal subspace): $k$-dimensional population factor subspace                                |
| $\Pi_B$, $\Pi_B^\perp$                | → §2 (Signal subspace): orthogonal projection onto $\mathcal{B}$ and its complement $I_p - \Pi_B$ |
| $\Sigma_0^{(p)} = B\Sigma_F B^\top/p$ | → §2 (Population loading direction): population signal covariance                                 |
| $\bar{b}_j$                           | → §2 (Population loading direction): $j$-th population loading direction                          |

**Prevalence-weighted factor covariances**

| Symbol                                 | Defined                                                                                                  |
|:--------------------------------------:|:-------------------------------------------------------------------------------------------------------- |
| $F^\# = C^{1/2}F^\top$                 | → §2 (Prevalence-rescaled factor returns)                                                                |
| $\hat{D} = C^{1/2}(F^\top F/n)C^{1/2}$ | → §2 (Prevalence-weighted factor covariance matrices)                                                    |
| $D = C^{1/2}\Sigma_F C^{1/2}$          | → §2 (Prevalence-weighted factor covariance matrices)                                                    |
| $\hat{M}$, $M$                         | → §2, equation (2): rotated-reweighted covariances; reduce to $\hat{D}$, $D$ when $G_\infty = I_k$       |
| $\rho_j$, $\hat{w}_j$                  | → §2 (Eigenvalues and eigenvectors of $\hat{M}$ and $M$): $j$-th eigenvalue and eigenvector of $\hat{M}$ |
| $w_j$                                  | → §2 (Eigenvalues and eigenvectors of $\hat{M}$ and $M$): $j$-th eigenvector of $M$                      |
| $d_j = \lim_{n\to\infty}\rho_j$        | → §2 (Eigenvalues and eigenvectors of $\hat{M}$ and $M$)                                                 |
| $\mathrm{SNR}_j = n\rho_j/\delta^2$    | → §2 (Eigenvalues and eigenvectors of $\hat{M}$ and $M$)                                                 |

**Sample PCA objects** (→ §2, Sample PCA objects, equation (3))

| Symbol                                           | Defined                                                                         |
|:------------------------------------------------:|:------------------------------------------------------------------------------- |
| $H = [h_1,\ldots,h_k]\in\mathbb{R}^{p\times k}$  | → §2: top-$k$ sample eigenvectors of $YY^\top/n$; orthonormal columns           |
| $S_p = \mathrm{diag}(s_{p,1},\ldots,s_{p,k})$    | → §2: diagonal matrix of top-$k$ singular values of $Y/\sqrt{n}$                |
| $\mathcal{X}_p = [\chi_{p,1},\ldots,\chi_{p,k}]$ | → §2: right singular vectors of $Y/\sqrt{n}$; orthonormal columns               |
| $s_{p,j}$                                        | → §2: $j$-th largest singular value; $s_{p,j}^2/p \to \rho_j + \delta^2/n$ a.s. |
| $\chi_{p,j}$                                     | → §2: $j$-th column of $\mathcal{X}_p$; converges to $v_j$ a.s.                 |
| $\Pi_H = HH^\top$                                | → §2 (Sample PCA objects): orthogonal projection onto $\mathrm{col}(H)$         |
| $W^{(p)} = Y^\top Y/(np)$                        | → §2 (Analytical strategy, Sample PCA objects): small Gram matrix               |

**Limiting small Gram matrix** (→ Appendix B.1, Lemma 7)

| Symbol                         | Meaning                                                                                    |
|:------------------------------:|:------------------------------------------------------------------------------------------ |
| $W_\infty$                     | Almost-sure spectral limit of $W^{(p)}$: $W_\infty = F\Gamma_B F^\top/n + (\delta^2/n)I_n$ |
| $\tau_j = \rho_j + \delta^2/n$ | $j$-th largest eigenvalue of $W_\infty$                                                    |
| $v_j$                          | $j$-th eigenvector of $W_\infty$: $(F^\#)^\top Q\Lambda_G^{1/2}\hat{w}_j/\sqrt{n\rho_j}$   |

**Shrinkage and alignment** (→ §5, Corollaries 1 and 5)

| Symbol                                                                | Meaning                                                                                 |
|:---------------------------------------------------------------------:|:--------------------------------------------------------------------------------------- |
| $\psi_{\infty,j}^2 = n\rho_j/(n\rho_j+\delta^2)$                      | Squared alignment ceiling for eigenvector $h_j$                                         |
| $\hat{\psi}_{p,j}^2 = 1 - \ell_p^2/s_{p,j}^2$                         | Observable estimator of $\psi_{\infty,j}^2$ (§5, Corollary 1)                           |
| $\ell_p^2 = \frac{1}{n-k}\sum_{j=k+1}^n s_{p,j}^2$                    | Mean squared noise singular value; $\to \delta^2/n$ a.s.                                |
| $\Psi_\infty = \mathrm{diag}(\psi_{\infty,1},\ldots,\psi_{\infty,k})$ | Diagonal shrinkage matrix (§5, Corollaries 5)                                           |
| $\tilde{B} = b(p)$                                                    | Unit loading column matrix (§5, Corollary 5; also §2)                                   |
| $\Gamma_\infty = \lim_{p\to\infty}\tilde{B}^\top W$                   | Asymptotic alignment matrix between unit loadings and probe frame $W$ (§5, Corollary 5) |

**Angles.** For unit vectors $u,v\in\mathbb{R}^p$: $\cos\angle(u,v) = |\langle u,v\rangle|$ and $\sin^2\angle(u,v) = 1 - \langle u,v\rangle^2$.

---

## Appendix B: Proofs

This appendix contains the proofs of all lemmas and both parts of the theorem. Appendix B.1 establishes the two foundational noise results (Lemmas 1 and 4 and Corollary 1.1). Appendix B.2 proves Part (i). Appendix B.3 proves Part (ii) in seven steps, working in $\Gamma_B$ coordinates throughout. Appendix B.4 remarks on the unified proof structure. Appendix B.5 recovers NG's Theorem 3.1+ as a special case.

### B.1 Lemmas

The proof rests on two fundamental noise results. **Lemma 1** is the main probabilistic workhorse: a fourth-moment Borel–Cantelli argument showing that a single noise projection $\eta_p^\top Z_{\cdot\ell}/\sqrt{p}$ vanishes a.s. for any bounded deterministic sequence $\eta_p$. **Lemma 4** shows the noise Gram matrix $Z^\top Z/p$ concentrates around $\delta^2 I_n$, proved by the Kolmogorov SLLN. **Corollary 1.1** (following Lemma 1) extends the pointwise bound to the full matrix $U(p)^\top Z$, giving $\Vert U(p)^\top Z\Vert_F^2/p \to 0$ a.s. by applying Lemma 1 column-by-column.
...
*Let $U(p)\in\mathbb{R}^{p\times k}$ be a deterministic sequence with orthonormal columns spanning $\mathcal{B}$. Then $\Vert U(p)^\top Z\Vert_F^2/p \to 0$ a.s. The same bound holds with $b(p)$ in place of $U(p)$: $\Vert b(p)^\top Z\Vert_F^2/p \to 0$ a.s.*

**Proof.** Each column $u_l(p)$ of $U(p)$ is a deterministic sequence of unit vectors. Lemma 1 applies to each pair $(l, m)$ with $l\in\{1,\ldots,k\}$, $m\in\{1,\ldots,n\}$:

$$
|u_l(p)^\top Z_{\cdot m}| = o(p^{1/2-\varepsilon}) \quad\text{a.s.}
$$

The $(l,m)$ entry of $U^\top Z$ equals $u_l^\top Z_{\cdot m}$, so summing the $kn$ squared terms:

$$
\Vert U^\top Z\Vert_F^2 = \sum_{l=1}^k\sum_{m=1}^n |u_l^\top Z_{\cdot m}|^2 = o(p^{1-2\varepsilon}) \quad\text{a.s.}
$$

Dividing by $p$ gives $\Vert U^\top Z\Vert_F^2/p \to 0$ a.s. The same argument applies to $b(p)$: each column $\beta_j/\Vert \beta_j\Vert $ is a deterministic unit vector, so Lemma 1 gives $|(\beta_j/\Vert \beta_j\Vert )^\top Z_{\cdot m}| = o(p^{1/2-\varepsilon})$ a.s., and summing $kn$ squared terms gives $\Vert b^\top Z\Vert_F^2/p \to 0$ a.s. $\square$

*Why Lemma 1 suffices (no Marcinkiewicz–Zygmund needed).* The elementary reason is that a rank-$k$ projector has bounded expected Frobenius norm: $\mathbb{E}\Vert \Pi_B Z\Vert_F^2 = \delta^2\mathrm{tr}(\Pi_B) = \delta^2 k$, independent of $p$. Lemma 1's fourth-moment Borel–Cantelli argument promotes this $O(1)$ expectation to almost-sure convergence of the normalized version $\Vert U^\top Z\Vert_F^2/p \to 0$.
...
Apply $\Pi_B^\perp$ to both sides of (3). The signal term vanishes: $\Pi_B^\perp B = 0$ (every column of $B$ lies in $\mathcal{B}$), so
...
$$
H^\top v - H^\top\Pi_B v \;=\; S_p^{-1}\,\frac{\mathcal{X}_p^\top Z^\top \eta_p}{\sqrt{n}}. \tag{6}
$$
...
*Lower bound on singular values.* By Weyl's inequality applied to $YY^\top/(np) = \Sigma_0^{(p)} + \text{noise}$, the $i$-th eigenvalue of $YY^\top/(np)$ satisfies $s_{p,i}^2/(np) \ge \lambda_i(\Sigma_0^{(p)}) - \Vert Z^\top Z/(np) - (\delta^2/n)I_p\Vert_{\mathrm{op}}$. The second term vanishes a.s. by Lemma 4. The $i$-th eigenvalue of $\Sigma_0^{(p)} = B\Sigma_F B^\top/p$ converges to a positive limit under Assumption 1 (it equals $c_{(i)}\sigma_{(i)}^2 + o(1)$ where the subscript denotes the $i$-th ordered value). Hence $s_{p,i}^2/p \ge c > 0$ a.s. for all large $p$, giving $s_{p,i}\asymp\sqrt{p}$ a.s.
...
*Note.* Part (i) requires only Assumption 1 and the noise assumptions. The projection $\Pi_B^\perp$ kills the signal identically — the argument is algebraic, independent of the loading correlation structure. The per-eigenvector results of Part (ii) additionally require Assumptions 2 and 3 to identify which part of $\mathcal{B}$ each sample eigenvector points toward.

---

### B.3 Proof of Part (ii)

The proof establishes Part (ii) for general positive-definite $G_\infty$ in a single argument. The key device is to work in the non-orthonormalized loading frame $\Phi_p(x) = Bx/\sqrt{p}$ rather than the SVD basis $U(p)$. Because $\Phi_p^\top\Phi_p = B^\top B/p = \Gamma_p \to \Gamma_B$ for the full sequence (Assumptions 1–2), the rotational ambiguity that would arise from coalescing singular values of $b(p)$ (when $G_\infty = I_k$) never appears. The proof proceeds in seven steps: (B.3.1) decompose the misalignment; (B.3.2–B.3.3) compute the limiting spectrum of the small Gram matrix $W^{(p)}$; (B.3.4) apply eigenprojection continuity to pass limits through eigenvectors; (B.3.5) establish convergence of both the sample and population in-subspace directions in $\Gamma_B$ coordinates; (B.3.6) compute the floor and in-subspace angle; (B.3.7) assemble. Steps B.3.2–B.3.4 operate on the fixed-size $n\times n$ matrix $W^{(p)}$, where finite-dimensionality makes spectral analysis tractable.

#### B.3.1 Parallel/Perpendicular Decomposition

Since $\bar{b}_j\in\mathrm{col}(U(p))$, write $h_j^\perp = (I-\Pi_U)h_j$ and $h_j^{\parallel} = \Pi_U h_j$. The angle-decomposition identity gives:

$$
\sin^2\angle(h_j,\,\bar{b}_j) \;=\; \Vert h_j^\perp\Vert^2 \;+\; \Vert h_j^{\parallel}\Vert^2\,\sin^2\!\angle\!\left(\frac{h_j^{\parallel}}{\Vert h_j^{\parallel}\Vert },\,\bar{b}_j\right). \tag{7}
$$

This follows from $\sin^2\angle(h,b) = 1-\langle h,b\rangle^2$, $\langle h_j^\perp, \bar{b}_j\rangle = 0$ (since $\bar{b}_j\in\mathrm{col}(U)$ and $h_j^\perp\perp\mathrm{col}(U)$), and $\Vert h_j^\perp\Vert^2 + \Vert h_j^{\parallel}\Vert^2 = 1$.

#### B.3.2 Expansion of $W^{(p)}$ and Its Limit

Define $\Gamma_B := C^{1/2}G_\infty C^{1/2}$. Substitute $Y = BF^\top + Z$ into $W^{(p)} = Y^\top Y/(np)$:

$$
W^{(p)} = \underbrace{\frac{F\,(B^\top B/p)\,F^\top}{n}}_{(\mathrm{A})} \;+\; \underbrace{\frac{F\,B^\top Z + Z^\top B\,F^\top}{np}}_{(\mathrm{B})} \;+\; \underbrace{\frac{Z^\top Z}{np}}_{(\mathrm{C})}. \tag{8}
$$
...
**Term (B).** Each column $b_j(p) = \beta_j/\Vert \beta_j\Vert $ of $b(p)$ is a deterministic unit vector. Corollary 1.1 gives $\Vert b(p)^\top Z\Vert_F^2/p \to 0$ a.s. Since $B^\top Z = A(p)\cdot b(p)^\top Z$ and $A(p)/\sqrt{p} \to C^{1/2}$ (bounded), $\Vert B^\top Z\Vert_F/(p\sqrt{n}) \to 0$ a.s., so term (B) vanishes in spectral norm.
...
$$
W^{(p)} \;\longrightarrow\; W_\infty \;:=\; \frac{F\,\Gamma_B\,F^\top}{n} \;+\; \frac{\delta^2}{n}\,I_n \quad\text{a.s. in spectral norm.} \tag{9}
$$

#### B.3.3 Eigenstructure of $W_\infty$
...
$$
v_j \;=\; \frac{F\,C^{1/2}Q\,\Lambda_G^{1/2}\hat{w}_j}{\sqrt{n\rho_j}} \;=\; \frac{(F^\#)^\top Q\,\Lambda_G^{1/2}\hat{w}_j}{\sqrt{n\rho_j}}, \quad j\in\{1,\ldots,k\}, \tag{10}
$$

*where $\hat{w}_j$ is the $j$-th eigenvector of $\hat{M}$ and $(F^\#)^\top = FC^{1/2}\in\mathbb{R}^{n\times k}$.* When $G_\infty = I_k$: $\hat{M} = \hat{D}$, $Q = I_k$, $\Lambda_G = I_k$, and (10) reduces to $v_j = (F^\#)^\top\hat{w}_j/\sqrt{n\rho_j}$.
...
*Step 1: Nonzero eigenvalues via AB/BA.* The nonzero eigenvalues of $(FP)(FP)^\top/n$ equal those of $(FP)^\top(FP)/n$ (AB/BA: if $ABx = \rho x$ with $\rho\ne 0$, then $BA(Ax) = \rho(Ax)$). Compute:

$$
\frac{(FP)^\top(FP)}{n} \;=\; P^\top\!\left(\frac{F^\top F}{n}\right)P \;=\; \Lambda_G^{1/2}Q^\top\cdot\hat{D}\cdot Q\Lambda_G^{1/2} \;=\; \hat{M},
$$

using $F^\top F/n = C^{-1/2}\hat{D}C^{-1/2}$ (from $\hat{D} = C^{1/2}(F^\top F/n)C^{1/2}$) and $P = C^{1/2}Q\Lambda_G^{1/2}$. Hence the nonzero eigenvalues of $W_\infty - (\delta^2/n)I_n$ are exactly $\rho_1,\ldots,\rho_k$.

*Step 2: Verify $v_j$ is an eigenvector.* Since $\hat{M}\hat{w}_j = \rho_j\hat{w}_j$, we have $(FP)^\top(FP)\hat{w}_j = n\rho_j\hat{w}_j$. Set $v_j = (FP)\hat{w}_j/\sqrt{n\rho_j}$. Then:

$$
\frac{(FP)(FP)^\top}{n}\,v_j \;=\; \frac{(FP)\,(FP)^\top(FP)\,\hat{w}_j}{n\sqrt{n\rho_j}} \;=\; \frac{(FP)\,n\rho_j\,\hat{w}_j}{n\sqrt{n\rho_j}} \;=\; \rho_j\,v_j.
$$

Expanding $FP = FC^{1/2}Q\Lambda_G^{1/2} = (F^\#)^\top Q\Lambda_G^{1/2}$ gives formula (10).

A key identity used in B.3.6: from $\hat{M}\hat{w}_j = \rho_j\hat{w}_j$, left-multiplying successively by $\Lambda_G^{-1/2}$ and $Q$ gives:

$$
\hat{D}\,Q\Lambda_G^{1/2}\hat{w}_j \;=\; Q\,\rho_j\,\Lambda_G^{-1/2}\hat{w}_j. \tag{$*$}
$$

*Step 3: Unit norm.* $\Vert v_j\Vert^2 = \hat{w}_j^\top(FP)^\top(FP)\hat{w}_j/(n\rho_j) = \hat{w}_j^\top\hat{M}\hat{w}_j/\rho_j = 1$.
...
This is the qualitative version only — no explicit angular bound is produced. The eigenvalue convergence alone follows from Weyl's inequality applied to $W^{(p)} \to W_\infty$: since $s_{p,j}^2/p$ is the $j$-th eigenvalue of $W^{(p)}$ and $\tau_j$ is the $j$-th eigenvalue of $W_\infty$, $|s_{p,j}^2/p - \tau_j| \le \Vert W^{(p)} - W_\infty\Vert_{\mathrm{op}} \to 0$ a.s.

Applying eigenprojection continuity with gap $\gamma = \rho_k > 0$, for each $j\in\{1,\ldots,k\}$:

$$
\frac{s_{p,j}^2}{p} \;\longrightarrow\; \tau_j = \rho_j + \frac{\delta^2}{n}, \qquad \chi_{p,j} \;\longrightarrow\; v_j \quad\text{a.s., up to sign.} \tag{11}
$$

#### B.3.5 The $\Gamma_B$-Coordinate Framework
...
The normalization $\Vert \bar{b}_j\Vert^2 = (a_j^{(p)})^\top\Gamma_p a_j^{(p)} = 1$ holds since $\bar{b}_j$ is a unit vector.
...
*Sample direction.* From (3), the in-subspace component of $h_j$ is $\Pi_B h_j = \Pi_B Y\chi_{p,j}/(\sqrt{n}s_{p,j})$. Using $Y = BF^\top + Z$ and $\Pi_B B = B$:

$$
\Pi_B h_j \;=\; \frac{BF^\top\chi_{p,j}}{\sqrt{n}\,s_{p,j}} \;+\; \frac{\Pi_B Z\chi_{p,j}}{\sqrt{n}\,s_{p,j}}.
$$

For the noise term: each column $b_l = \beta_l/\Vert \beta_l\Vert $ of $b(p)$ is a deterministic unit vector, so Corollary 1.1 gives $\Vert b(p)^\top Z\Vert_F^2/p \to 0$ a.s., hence $\Vert \Pi_B Z\chi_{p,j}\Vert /\sqrt{p} \to 0$ a.s. Since $s_{p,j} \asymp \sqrt{p}$ a.s. (from B.3.4), this noise term is $o(1)$ a.s.
...
*Convergence of $g_j^{(p)}$.* Since $s_{p,j}^2/p \to \rho_j + \delta^2/n$ a.s. and $\chi_{p,j} \to v_j$ a.s. by (11), we have $\sqrt{p}/s_{p,j} \to 1/\sqrt{\rho_j + \delta^2/n}$ a.s. and $F^\top\chi_{p,j} \to F^\top v_j$. Hence:

$$
g_j^{(p)} \;\longrightarrow\; g_j^\infty \;:=\; \frac{F^\top v_j}{\sqrt{n\rho_j + \delta^2}} \quad\text{a.s.}
$$

This limit holds for the **full sequence**: every factor in (11) converges without subsequence extraction.

#### B.3.6 Floor and In-Subspace Angle via $\Gamma_B$ Inner Products
...
$$
\Vert \Pi_B h_j\Vert^2 \;=\; (g_j^{(p)})^\top\Gamma_p\,g_j^{(p)} + o(1) \;\longrightarrow\; (g_j^\infty)^\top\Gamma_B\,g_j^\infty \quad\text{a.s.}
$$
...
The following three steps evaluate these inner products explicitly, using the formula for $v_j$ from (10).

*Step 1: Compute $g_j^\infty$ explicitly.* From (10), $v_j = FC^{1/2}Q\Lambda_G^{1/2}\hat{w}_j/\sqrt{n\rho_j}$. Apply $F^\top$ (dimension: $k\times n$) to get $F^\top v_j \in \mathbb{R}^k$:
...
$$
g_j^\infty \;=\; \sqrt{\frac{n\rho_j}{n\rho_j+\delta^2}}\cdot C^{-1/2}Q\Lambda_G^{-1/2}\hat{w}_j. \tag{12}
$$
...
So $\Vert \Pi_B h_j\Vert^2 \to n\rho_j/(n\rho_j+\delta^2)$ a.s. The floor then follows from $\Vert h_j\Vert  = 1$:

$$
\Vert h_j^\perp\Vert^2 \;=\; 1 - \Vert \Pi_B h_j\Vert^2 \;\longrightarrow\; \frac{\delta^2}{n\rho_j+\delta^2} \quad\text{a.s.} \tag{13}
$$
...
*Step 4: In-subspace angle.* Using (7) and the limits from Steps 2–3:

$$
\begin{align}
\sin^2\!\angle\!&\left(\frac{\Pi_B h_j}{\Vert \Pi_B h_j\Vert },\,\bar{b}_j\right) = 1 - \frac{\langle\Pi_B h_j, \bar{b}_j\rangle^2}{\Vert \Pi_B h_j\Vert^2}\notag\\ &\longrightarrow\; 1 - \frac{(n\rho_j/(n\rho_j+\delta^2))\cdot(\hat{w}_j^\top w_j)^2}{n\rho_j/(n\rho_j+\delta^2)} \;=\; \sin^2\!\angle(\hat{w}_j,\,w_j).\notag
\end{align}
$$

*Diagonal-Gram specialization.* When $G_\infty = I_k$: $Q = I_k$, $\Lambda_G = I_k$, $w_j = e_j$ (since $M = D$ is diagonal under the ordering Assumption 3). Then $a_j^\infty = C^{-1/2}e_j = e_j/\sqrt{c_j}$, $\hat{w}_j^\top w_j = (\hat{w}_j)_j$, and $\sin^2\angle(\hat{w}_j, w_j) = \sin^2\angle(\hat{w}_j, e_j) = 1 - (\hat{w}_j)_j^2$, recovering the $G_\infty = I_k$ specialization stated after formula (5).

#### B.3.7 Assembly

Substituting (13) and Step 4 of B.3.6 into the decomposition (7):

$$
\sin^2\angle(h_j,\,\bar{b}_j) \;=\; \Vert h_j^\perp\Vert^2 \;+\; \Vert \Pi_B h_j\Vert^2\cdot\sin^2\!\angle\!\left(\frac{\Pi_B h_j}{\Vert \Pi_B h_j\Vert },\,\bar{b}_j\right)
$$
...
This is formula (5). All limits hold for the full sequence — no subsequence extraction is needed, because the $\Gamma_B$-coordinate framework operates directly on $\Gamma_p \to \Gamma_B$, bypassing the rotational ambiguity of $V(p)$ in the SVD basis. $\square$

---

### B.4 Remark on proof structure

The proof of Appendix B.3 treats $\Gamma_B = C^{1/2}G_\infty C^{1/2}$ as an arbitrary positive-definite matrix throughout — the $G_\infty = I_k$ specialization requires no separate argument. The $G_\infty = I_k$ case simply sets $\Gamma_B = C$, $\hat{M} = \hat{D}$, $M = D$, $w_j = e_j$, and $\sin^2\angle(\hat{w}_j, w_j) = \sin^2\angle(\hat{w}_j, e_j) = 1 - (\hat{w}_j)_j^2$.

In the earlier proof architecture (following the SVD basis $U(p)$), the diagonal and general cases required separate treatment because $V(p)$ — the right singular factor of $b(p) = U(p)\Sigma(p)V(p)^\top$ — has a rotational ambiguity when $G_\infty = I_k$ makes the singular values of $b(p)$ coalesce. Working in $\Gamma_B$ coordinates removes this artifact entirely: $\Gamma_p = B^\top B/p$ converges to $\Gamma_B$ for the full sequence, independently of any singular-value ordering, and neither the SVD of $b(p)$ nor the matrix $V(p)$ ever appears in the argument. $\square$

---

### B.5 Recovery of NG's Theorem 3.1+
...
**Assumption 2.6+ kills the in-subspace rotation.** Under 2.6+, factor returns are orthogonal: $F_i^\top F_j = 0$ for $i\ne j$. This makes $F^\top F/n$ diagonal: $(F^\top F/n)_{ij} = (\Vert F_j\Vert^2/n)\delta_{ij}$. Hence $\hat{D} = C^{1/2}(F^\top F/n)C^{1/2} = \mathrm{diag}(c_j\Vert F_j\Vert^2/n)$ is diagonal, with eigenvectors $\hat{w}_j = e_j$. Similarly $D = \mathrm{diag}(c_j\sigma_j^2)$ is diagonal with $w_j = e_j$. Therefore $\sin^2\angle(\hat{w}_j, e_j) = 0$ and the in-subspace term vanishes.
...
With $G_\infty = I_k$ and $\sin^2\angle(\hat{w}_j, e_j) = 0$, formula (5) reduces to:

$$
\sin^2\angle(h_j, \bar{b}_j) \;\to\; \frac{\delta^2}{n\rho_j + \delta^2},
$$

where $\rho_j = c_j\Vert F_j\Vert^2/n$. Using NG's notation $\alpha_j^2 := c_j$ and $|X_j|^2 := \Vert F_j\Vert^2$, this is $\delta^2/(\alpha_j^2|X_j|^2+\delta^2)$. Since $\bar{b}_j \to b_j := \beta_j/\Vert \beta_j\Vert $ under 2.5+:
